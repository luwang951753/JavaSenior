...
public class RefliectionTest {
    ...
    public void test2() throws Exception{
        ...
        Object obj = cons.newInstance("Tow", 12);
        Person p = (Person) obj;
     ...
     * 1.类的加载过程
     * 程序通过javac.exe命令以后，会生成一个或多个字节码文件(.class结尾)
     * 接着我们使用java.exe命令对某个字节码文件进行解释运行。相当于某个字节码文件加载到内存中，
     * 此过程就称为类的加载。加载到内存中的类，我们就称为运行时类，此运行时类，就作为Class的一个实例
     *
     * 2.换名话说，Class的实例就对应着一个运行时类
     *
     * 3.载到内存中的运行时类，会缓存一定的时间。在此时间之内，我们可以通过不同的方式
     *   来获取此运行时类
     */
